...
var infowindowGlobal = null;
var markersObservable = null;
...
var Location = function(locationElement) {
    ...
    infowindowGlobal = AddInfoWindow();
    ...
    function changeInfowindow(content) {
        infowindowGlobal.setContent(content);                         
    }
    
};

//Location.markerDisable = function() {
...
function ViewModel() {
    ...
    self.searchInput = ko.observable("Search Location");
    
    model.markers.forEach(function(locationObject) {
        self.markersObservable.push(new Location(locationObject));
    });
    ...
    self.initialInputClick = true; 
    self.inputClick = function() {
        if(self.initialInputClick) {        // Just erase default text on first click. Don't erase text after second click.
            self.initialInputClick = false;
            self.searchInput("");
        }
    };
...
function disableAll() {
    markersObservable().forEach(function(locationInstance) {
        locationInstance.markerDisable();
    });
}
